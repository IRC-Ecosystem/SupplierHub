    ...
    <div class="bg-white rounded-xl border border-slate-200 shadow-sm p-5 flex flex-col md:flex-row justify-between items-start md:items-center gap-4 hover:border-emerald-500 transition-colors">
        <div>
            <div class="flex items-center gap-3 mb-1">
                <span class="px-2 py-0.5 bg-emerald-100 text-emerald-700 text-xs font-bold rounded"><?= strtoupper(str_replace('_',' ',$o['status'])) ?></span>
        ...
        <div class="flex flex-wrap gap-2">
            <?php if ($o['status']==='paid'): ?><button onclick="fulfillmentAction(<?= $o['id'] ?>,'processing')" class="px-4 py-2.5 bg-amber-50 text-amber-700 border border-amber-200 rounded-lg text-sm font-bold"><i class="ph ph-gear mr-1"></i> Mulai proses</button><?php endif; ?>
            <?php if (in_array($o['status'],['paid','processing'],true)): ?><button onclick="shipOrder(<?= $o['id'] ?>)" class="px-4 py-2.5 bg-blue-50 text-blue-700 border border-blue-200 rounded-lg text-sm font-bold"><i class="ph ph-truck mr-1"></i> Catat pengiriman</button><?php endif; ?>
            <button onclick="viewReceipt(<?= $o['id'] ?>, '<?= $o['order_code'] ?>')" class="inline-flex items-center px-4 py-2.5 bg-indigo-50 text-indigo-700 border border-indigo-200 rounded-lg text-sm font-bold"><i class="ph ph-receipt mr-1"></i> Bukti bayar</button>
        </div>
    </div>
<?php endforeach; ?>
</div>
...
async function approveOrder(){if(!currentOrderId)return;const eta=document.getElementById('fulfillment-eta')?.value;if(eta){const e=await apiCall(BASE+'/api/orders.php?action=supplier_fulfillment','POST',{order_id:currentOrderId,fulfillment_action:'estimate',fulfillment_eta:eta});if(e.status!=='success'){showToast(e.message,'error');return;}}showToast('Memproses...','info');closeOrderModal();const r=await apiCall(BASE+'/api/orders.php?action=approve','POST',{order_id:currentOrderId});if(r.status==='success'){showToast(r.message);setTimeout(()=>location.reload(),1000);}else showToast(r.message,'error');}

async function fulfillmentAction(orderId, action, extra={}) { const r=await apiCall(BASE+'/api/orders.php?action=supplier_fulfillment','POST',{order_id:orderId,fulfillment_action:action,...extra}); showToast(r.message,r.status==='success'?'success':'error'); if(r.status==='success')setTimeout(()=>location.reload(),700); }
async function shipOrder(orderId){const ref=prompt('Masukkan referensi pengiriman lokal:');if(!ref)return;await fulfillmentAction(orderId,'shipped',{tracking_reference:ref});}
